...
export default function SocietyCard({society,index}:{society:Society,index:number}) {
    ...
    return (
        <Card onClick={()=>history.push('/'+society.slug)} className={'my-3 pointer-on-hover '+(index%2===0?" text-center text-md-left ":" text-center text-md-right ")+(theme==="dark"?"bg-dark border-light":"shadow-2")}>
            <Card.Body className={'d-flex flex-wrap flex-md-nowrap '+(index%2===0?"justify-content-start":"justify-content-end")}>
                {index%2===0&&society.gallery.length>0&&<img className="my-3 m-md-0" height={200} src={society.gallery[0]} alt={society.name}/>}
